class AssetConfig {
  static String iconPath = 'assets/icons';
  static String backgroundPath = 'assets/backgrounds';

  static String appLogo = "$iconPath/app_logo.png";
  static String user_icon_square = "$iconPath/user_icon_square.png";
  static String user_icon_rounded = "$iconPath/user_icon_rounded.png";
  static String splashBgMain = "$backgroundPath/splash_bg.png";
  static String splashBgMiddleCircle = "$backgroundPath/splash_bg_middle_circle.png";
  static String splashBgTopCircle = "$backgroundPath/splash_bg_top_circle.png";
  static String three_dot_icon = "$iconPath/three_dot_icon.png";
  static String like_icon2 = "$iconPath/like_icon2.png";
  static String like_icon = "$iconPath/like_icon.png";
  static String love_icon = "$iconPath/love_icon.png";
  static String love_react2_icon = "$iconPath/love_react2_icon.png";
  static String wow_icon = "$iconPath/wow_icon.png";
  static String sad_icon = "$iconPath/sad_icon.png";
  static String logout_icon = "$iconPath/logout_icon.png";
  static String haha_icon = "$iconPath/haha_icon.png";
  static String community_icon = "$iconPath/community_icon.png";
  static String comment_send_icon = "$iconPath/comment_send_icon.png";
  static String comment_icon_filled = "$iconPath/comment_icon_filled.png";
  static String comment_icon = "$iconPath/comment_icon.png";
  static String care_icon = "$iconPath/care_icon.png";
  static String angry_icon = "$iconPath/angry_icon.png";

}