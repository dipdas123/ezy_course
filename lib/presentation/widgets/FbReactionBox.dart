...
import '../../core/entities/reaction_emoji.dart';
import '../../utils/asset_constants.dart';

class FbReactionBox extends StatefulWidget {
  const FbReactionBox({super.key});
...
class FbReactionBoxState extends State<FbReactionBox> with TickerProviderStateMixin {
  ...
  final _durationLongPress = const Duration(milliseconds: 250);
  ...
  void _initAnimationBoxAndEmojis() {
    ...
    _moveRightGroupEmoji = Tween(begin: 0.0, end: 10.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.0, 1.0)),
    );
    ...
    _fadeInBox = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.7, 1.0)),
    );
    ...
    _pushEmojiLikeUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.0, 0.5)),
    );
    ...
    _pushEmojiLoveUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.1, 0.6)),
    );
    ...
    _pushEmojiHahaUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.2, 0.7)),
    );
    ...
    _pushEmojiWowUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.3, 0.8)),
    );
    ...
    _pushEmojiSadUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.4, 0.9)),
    );
    ...
    _pushEmojiAngryUp = Tween(begin: 30.0, end: 60.0).animate(
      CurvedAnimation(parent: _animControlBox, curve: const Interval(0.5, 1.0)),
    );
  ...
  String _getImageEmojiBtn() {
    if (!_isLongPress && _isLiked) {
      return AssetConfig.icLikeFill;
    } else if (!_isDragging) {
      switch (_emojiUserChoose) {
        case ReactionEmoji.nothing:
          return AssetConfig.icLike;
        case ReactionEmoji.like:
          return AssetConfig.icLikeFill;
        case ReactionEmoji.love:
          return AssetConfig.icLove2;
        case ReactionEmoji.haha:
          return AssetConfig.icHaha2;
        case ReactionEmoji.wow:
          return AssetConfig.icWow2;
        case ReactionEmoji.sad:
          return AssetConfig.icSad2;
        case ReactionEmoji.angry:
          return AssetConfig.icAngry2;
      }
    } else {
      return AssetConfig.icLike;
    }
  ...
  void _handleWhenDragBetweenEmoji(ReactionEmoji currentEmoji) {
    _playSound(AudioConstant.focus);
  ...
  void _onTapUpBtn(TapUpDetails? tapUpDetail) {
    if (_isLongPress) {
      if (_emojiUserChoose == ReactionEmoji.nothing) {
        _playSound(AudioConstant.boxDown);
      } else {
        _playSound(AudioConstant.pick);
      }
    }
  ...
  void _onTapBtn() {
    if (!_isLongPress) {
      ...
      if (_isLiked) {
        _playSound(AudioConstant.shortPressLike);
  ...
  void _showBox() {
    _playSound(AudioConstant.boxUp);
    _isLongPress = true;
